...
class BinaryTree{
  ...
  public:
    ...
    bool IsIdenticalTo(BinaryTree* subtree){
      if(subtree ==  NULL)
        return true;
      if(value  == subtree->value){
        return left->IsIdenticalTo(subtree->left) && right->IsIdenticalTo(subtree->right);
      }
      return false;
    }

    bool HasSubtree(BinaryTree* subtree){
      if(subtree == NULL)
        return false;
      bool ans = false;
      if(value ==  subtree->value)
        ans = IsIdenticalTo(subtree);
      if(left != NULL)
        ans = ans || left->HasSubtree(subtree);
      if(right != NULL)
        ans = ans || right->HasSubtree(subtree);
      return ans;
    }
};
...
int main()
{
  int array1[] ={7,8,9,10,12,14,15,16};
  BinaryTree *bt1 = BinaryTree::convertArrayToBinaryTree(array1,sizeof(array1)/sizeof(array1[0]));
  bt1->print_inorder();
  NL;

  int array2[] ={8,9,10};
  BinaryTree *bt2 = BinaryTree::convertArrayToBinaryTree(array2,sizeof(array2)/sizeof(array2[0]));
  bt2->print_inorder();
  NL;
  
  cout << bt1->HasSubtree(bt2) << endl;
